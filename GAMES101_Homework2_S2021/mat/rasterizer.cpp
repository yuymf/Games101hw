...
{   
    ...
    if ((p1 - p0).cross(q - p0).z() > 0){
        return (p2 - p1).cross(q - p1).z() > 0 && (p0 - p2).cross(q - p2).z() > 0;
    }
    else {
        return (p2 - p1).cross(q - p1).z() < 0 && (p0 - p2).cross(q - p2).z() < 0;
    }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    auto v = t.toVector4();
    float alpha, beta, gamma, lmin = INT_MAX, rmax = INT_MIN, bmin = INT_MAX, tmax = INT_MIN;
    for (auto& k : v){
