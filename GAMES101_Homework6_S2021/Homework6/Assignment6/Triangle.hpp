...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    double det = dotProduct(e1, pvec); // SE
    if (fabs(det) < EPSILON)
        return inter;

    double det_inv = 1. / det;
    Vector3f tvec = ray.origin - v0; //S
    u = dotProduct(tvec, pvec) * det_inv; //u
    if (u < 0 || u > 1)
        return inter;
    Vector3f qvec = crossProduct(tvec, e1); //S2
    v = dotProduct(ray.direction, qvec) * det_inv; //v
    if (v < 0 || u + v > 1)
        return inter;
    ...
    inter.happened = true;
    inter.coords = ray(t_tmp); //ray.origin + t_temp * ray.direction
    inter.normal = normal;
