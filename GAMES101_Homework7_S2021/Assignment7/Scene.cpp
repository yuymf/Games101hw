...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
    ...
    float pdf_light = 0.0f; // init after lightinit!!!
    Scene::sampleLight(lightinter, pdf_light);

    Vector3f NN = lightinter.normal;
    Vector3f x = lightinter.coords;
    Vector3f emit = lightinter.emit; // l_dir's emit is lightinter!!!!

    Vector3f p = inter.coords;
    Vector3f N = inter.normal;

    // float xpinv = 1 / ((x - p).norm() * (x - p).norm());
    float xpinv = dotProduct((x - p), (x - p));
    Vector3f ws = (x - p).normalized();
    Ray o2l(inter.coords, ws);
    Intersection o2linter = Scene::intersect(o2l);

    if (o2linter.distance - (x - p).norm() > -EPSILON)
    {
        l_dir = emit * inter.m->eval(ray.direction, ws, N) * dotProduct(-ws, NN) * dotProduct(ws, N) / xpinv / pdf_light; // -ws!!!
    }
    ...
    Vector3f wi = inter.m->sample(ray.direction, N).normalized(); // notice to normalized
    Ray diff(p, wi);
    Intersection diffinter = Scene::intersect(diff);
    if (diffinter.happened && !diffinter.m->hasEmission())
    {
        float pdf = inter.m->pdf(ray.direction, wi, N);
        l_indir = castRay(diff, depth + 1) * inter.m->eval(ray.direction, wi, N) * dotProduct(wi, N) / pdf / RussianRoulette;
    }
